 ...
 * ---------------------------------------------------
 * 11/26/2015       dpang       Issue 164029. Add PartyMergeProcessor for client merge
 * ---------------------------------------------------
 */
public class PartyMergeProcessor extends BasePartyChangeProcessor {
    ...
    public void process(PartyChangeRequestType partyChangeRequest, PartyChangeResultType partyChangeResult) {
        ...
        String errorMsg = getEntityMntDuplicateManager().saveEntityMntDuplicateWs(prevRecord);

        //propagate oracle error message if exists
        if (!StringUtils.isBlank(errorMsg)) {
            MessageManager.getInstance().addErrorMessage("ci.webservice.error", new Object[]{errorMsg});
            throw new ValidationException(errorMsg);
        }

        PartyInquiryResultType party = getPartyInfo(entityId);
    ...
    public EntityMntDuplicateManager getEntityMntDuplicateManager() {
        return m_entityMntDuplicateManager;
    }

    public void setEntityMntDuplicateManager(EntityMntDuplicateManager entityMntDuplicateManager) {
        this.m_entityMntDuplicateManager = entityMntDuplicateManager;
    }

    private EntityMntDuplicateManager m_entityMntDuplicateManager;

    private static final String Y = "Y";
    private static final String ENTITY_FROM = "entityFrom";
    private static final String ENTITY_TO = "entityTo";
