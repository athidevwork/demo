 ...
 * ---------------------------------------------------
 */
public class HubPropertyChangeProcessor {
        ...
        if (partyChangeRequest != null && partyChangeRequest.getProperty().size() > 0) {
            ...
            for (PropertyType changedProperty : partyChangeRequest.getProperty()) {
                ...
                if (rowStatus.equals(ROW_STATUS_NEW) || rowStatus.equals(ROW_STATUS_MODIFIED)) {
                    ...
                    Record result = getHubPartyManager().saveHubParty(changedPropertyRecord);

                    if (ROW_STATUS_NEW.equals(rowStatus)) {
                        changedProperty.setPropertyNumberId(result.getStringValue("newPropertyId"));
                    }
                }
            }
