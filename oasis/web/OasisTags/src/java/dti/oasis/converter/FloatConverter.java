...
/**
 * Converts the given value from it's type to a Float.
...
public class FloatConverter extends BaseConverter {
     ...
     * @throws IllegalArgumentException if neither the inputValue nor the targetType is of type Float.
     */
    public Object convert(Class targetType, Object inputValue, Object nullValue) throws NumberFormatException, IllegalArgumentException {
        ...
        if (Float.class.isAssignableFrom(targetType) || Float.TYPE.isAssignableFrom(targetType)) {
            if (inputValue instanceof Float) {
                ...
                convertedValue = new Float(((Number) inputValue).floatValue());
            } else if (inputValue == null || inputValue.toString().equalsIgnoreCase("")) {
                convertedValue = nullValue;
            } else {
                convertedValue = Float.valueOf(inputValue.toString());
            }
        } else {
            throw new IllegalArgumentException("The FloatConverter requires the targetType to be a java.lang.Float. Received targetType<" + targetType.getName() + ">");
        }
