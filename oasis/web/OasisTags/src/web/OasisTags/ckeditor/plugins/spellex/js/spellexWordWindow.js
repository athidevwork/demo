//*********************************************************************
// spellexWordWindow.js
// Purpose: Functions used by Spell Checker Word Window
//*********************************************************************

/**
 Revision Date    Revised By  Description
 -----------------------------------------------------------------------------
 01/30/2013       htwang      issue 139459, replace charAt() method with substring() method
                                          to extracts long characters
**/
...
function setFocus( textIndex, wordIndex ) {
    //alert('setFocus('+textIndex+', '+wordIndex+')');
    var word = this._getWordObject( textIndex, wordIndex );
	if( word ) {
		if( word.type == "text" ) {
			word.focus();
...
function removeFocus( textIndex, wordIndex ) {
    //alert('removeFocus('+textIndex+', '+wordIndex+')');
    var word = this._getWordObject( textIndex, wordIndex );
	if( word ) {
        if( word.type == "text" ) {
			word.blur();
...
function setText( textIndex, wordIndex, newText ) {
	...
	if( word ) {
        ...
        //alert('word.value: '+word.value+' newText: "'+newText+'"');
...
//            alert('orig: '+orig);
			...
			for( var i = 0; i < orig.length; i++ ) {
				...
				do {
                    //MG
                    var problemType = this.problems[txtid][i];
//                    alert('problemType: '+problemType);
                    var position = this.positions[txtid][i];
                    if(problemType == 'D')
                    begin_idx = position;
                    else
                ...
                d.write( this.printForHtml( wordtxt.substring(this._lastPos( txtid, i ), begin_idx)));
