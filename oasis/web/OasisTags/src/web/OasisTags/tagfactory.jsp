<%--
    1. This page should be included via an include directive
    <%@ include file="tagfactory.jsp"%>
...
//    String onChange = "handleOnChange(this)";
    ...
    if (type.equals("CHECKBOX")) {
        ...
        if (!field.getIsReadOnly() && field.getIsVisible()) {
            try {
                checkBoxSpan = (String) tagUtils.lookup(pageContext, field.getFieldId() + "CheckBoxSpan", null);
                if (checkBoxSpan == null)
                    checkBoxSpan = (String) tagUtils.lookup(pageContext, "CheckBoxSpan", null);
                if (checkBoxSpan == null)
                    checkBoxSpan = "1";
            }
            catch (ClassCastException ce) {
...
<oweb:checkbox fieldColSpan="<%=checkBoxSpan%>" oasisFormField="<%=field%>" listOfValues="<%=lov%>"
               tabindex="<%=field.getTaborder()%>"
