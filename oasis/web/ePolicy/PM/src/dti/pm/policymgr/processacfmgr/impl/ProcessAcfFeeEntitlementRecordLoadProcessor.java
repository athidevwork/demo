 ...
 * ---------------------------------------------------
 */
public class ProcessAcfFeeEntitlementRecordLoadProcessor extends DefaultRecordLoadProcessor {
    ...
    public boolean postProcessRecord(Record record, boolean rowIsOnCurrentPage) {
        record.setFieldValue(IS_FEE_DEL_AVAILABLE, YesNoFlag.N);
        // Set Delete option availability, enabled when the record is created by the current transaction and it is still in WIP
        if (ProcessAcfFields.getFeeTransId(record).endsWith(getPolicyHeader().getLastTransactionId()) &&
            getPolicyHeader().getLastTransactionInfo().getTransactionStatusCode().isInProgress()) {
            record.setFieldValue(IS_FEE_DEL_AVAILABLE, YesNoFlag.Y);
        }
        return true;
    }
    ...
    public void postProcessRecordSet(RecordSet recordSet) {
        ...
        if (recordSet.getSize() == 0) {
            ...
            pageEntitlementFields.add(IS_FEE_DEL_AVAILABLE);
            recordSet.addFieldNameCollection(pageEntitlementFields);
        }
        recordSet.getSummaryRecord().setFieldValue(IS_FEE_ADD_AVAILABLE, YesNoFlag.N);
        ...
        if (getPolicyHeader().getLastTransactionInfo().getTransactionStatusCode().isInProgress()) {
            recordSet.getSummaryRecord().setFieldValue(IS_FEE_ADD_AVAILABLE, YesNoFlag.Y);
        }

        // Set readOnly attribute to summary record
        if (isRecordSetReadOnly()) {
            recordSet.setFieldValueOnAll(IS_FEE_DEL_AVAILABLE, YesNoFlag.N);
    ...
    public ProcessAcfFeeEntitlementRecordLoadProcessor(PolicyHeader policyHeader) {
    ...
    public static final String IS_FEE_ADD_AVAILABLE = "isFeeAddAvailable";
    public static final String IS_FEE_DEL_AVAILABLE = "isFeeDelAvailable";
}
