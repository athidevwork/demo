...
prompt ******* Extract Data
SELECT xpi.xpd_output_interface_pk, xpi.request_id, xpi.term_base_fk, xpi.transaction_log_pk, xpi.status_time, xpi.xml_file, xpi.zipped_xml, xpi.status 
FROM xpd_output_interface xpi, transaction_log tl
where tl.transaction_log_pk = xpi.transaction_log_pk
and policy_fk = 
       ( select policy_pk
           from policy
          where policy_no = upper('&policyno'))
ORDER BY xpi.status_time DESC
/
